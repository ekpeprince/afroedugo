...
const MainMenu = ({ onNavigate }) => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      label: 'Find Housing',
      sub: 'Premium Upgrade',
      color: 'bg-primary/10 text-primary dark:bg-gray-800 dark:text-gray-300',
      shadow: 'shadow-none',
      comingSoon: true,
      icon: (
        <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="min-h-screen bg-transparent dark:bg-gray-900 transition-colors duration-300 p-6 pb-32 flex flex-col max-w-5xl mx-auto">
      ...
      {/* PWA Install Banner */}
      {showInstallBanner && (
        <div className="mx-6 mt-4 bg-gradient-to-r from-primary to-primary-light text-white p-4 rounded-2xl shadow-xl shadow-primary/20 border border-primary-light/30 flex items-center justify-between animate-in fade-in slide-in-from-top-4">
          <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
        {menuItems.map((item) => (
          ...
          >
            <div className={`w-16 h-16 ${item.color} ${item.shadow} rounded-2xl flex items-center justify-center mb-6 shadow-lg dark:shadow-none ${!item.comingSoon && 'group-hover:rotate-6 transition-transform'}`}>
              {item.icon}
            </div>
